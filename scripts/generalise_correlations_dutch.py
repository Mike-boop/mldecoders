...
def study(participant, test_batch_size, condition='fM', parts=range(1,5), data_dir=""):
    data = OctaveMapped(50,participant, condition, parts, data_dir=data_dir)
    ...
    eegnet = EEGNet.load_from_checkpoint(checkpoint_path=eegnet_checkpoint_path,**eegnet_config, data_dir="/home/mdt20/Code/hugo_data_processing/processed_data/250321/data.h5")
    eegnet.eval()
    eegnet.to(device='cuda')
    detaillez = DeTaillez.load_from_checkpoint(checkpoint_path=detaillez_checkpoint_path,**detaillez_config, data_dir="/home/mdt20/Code/hugo_data_processing/processed_data/250321/data.h5")
    detaillez.eval()
    ...
    corrs["ridge_attended_null"] = ridge.score_in_batches(ridge_eeg.T, ridge_stim[1][::-1, None], batch_size=test_batch_size).tolist()
    corrs["ridge_unattended"] = ridge.score_in_batches(ridge_eeg.T, ridge_stim[0][:, None], batch_size=test_batch_size).tolist()
    corrs["ridge_unattended_null"] = ridge.score_in_batches(ridge_eeg.T, ridge_stim[0][::-1, None], batch_size=test_batch_size).tolist()
...
if __name__ == '__main__':
    ...
    participants = ["YH07", "YH08", "YH09", "YH10", "YH11", "YH12", "YH13",
                    "YH14", "YH15", "YH16", "YH17", "YH18", "YH19", "YH20"]
    ...
    for batch_size in [250]:
        ...
        json.dump(results, open(os.path.join(resultsdir,"generalisation/{}_dutch_correlations.json".format(batch_size)), "w"))
