...
results_path = pathlib.Path('results/2-8Hz')
ground_truth_path = pathlib.Path('results/0.5-8Hz')
...
for participant in range(3):
    x_cnn = np.load(os.path.join(results_path, 'predictions', 'hugo_subject_specific', f'cnn_predictions_P{participant:02d}.npy'))
    x_fcnn = np.load(os.path.join(results_path, 'predictions', 'hugo_subject_specific', f'fcnn_predictions_P{participant:02d}.npy'))
    x_ridge = np.load(os.path.join(results_path, 'predictions', 'hugo_subject_specific', f'ridge_predictions_P{participant:02d}.npy'))
    y = np.load(os.path.join(ground_truth_path, 'predictions', 'hugo_subject_specific', 'ground_truth.npy'))
