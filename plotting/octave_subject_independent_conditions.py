...
for participant in english_participants:
    ...
    clean_ground_truth = np.load(os.path.join(results_path, f'attended_ground_truth_{participant}_clean.npy'))
    ...
    for model in models:
        ...
        clean_predictions = np.load(os.path.join(results_path, f'{model}_predictions_{participant}_clean.npy'))
        ...
        clean_scores = mb_scores[~np.isnan(mb_scores)]
        ...
        conditions_mean_scores['clean'][model].append(np.mean(clean_scores))
...
for i, cond in enumerate(['clean', 'babble', 'dutch', 'competing-attended']):
